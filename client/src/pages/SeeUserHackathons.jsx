...
const UserHackathons = () => {
    ...
    React.useEffect(() => {
                const fetchHackathonData = async (username) => {
                    try {
                        const response1 = await fetch(`${VITE_API_URL}/api/users/profile/${username}/participationHistory`);
                        const data1 = await response1.json();
                        setPastRegistrations(data1);
                        const response2 = await fetch(`${VITE_API_URL}/api/users/profile/${username}/futureParticipations`);
    ...
    return (
        <div className="flex w-screen text-center w-400 h-150 flex-col bg-[#191919] text-white">
            ...
            <div className= "flex-10 pl-10 mb-10">
                <h2 className = " text-[24px] text-left font-medium mb-5">Mis inscripciones</h2>
                <div className="flex gap-20 h-44 w-full flex-row">
                {futureRegistrations.data?.user?.map(
                    (hackathon, index) => (
                        <div
                            key={index}
            ...
            <div className= "flex-10 pl-10 text-[24px] text-left font-medium">
                ...
                {pastRegistrations.data?.user?.map(
                    (hackathon, index) => (
                    <div
                        key={index}

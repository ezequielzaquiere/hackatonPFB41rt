                    ...
                    await Promise.all(
                        data1.data?.user?.map(async (hackathon) => {
                            try {
                                ...
                                console.error(`Error encontrando rating para el hackathon con id ${hackathon.id}`, err);
                            }
                        })
                    );
    ...
    return (
        <main className="flex min-h-screen items-center text-center px-8 py-12 lg:px-20 lg:py-20 flex-col bg-[#191919]">
            ...
            <div className="w-full max-w-4xl mt-16">
                <h2 className="text-2xl text-white font-semibold mb-6">Mi historial</h2>
                <div className= "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {pastRegistrations.data?.user?.map((hackathon, index) => (
                        <div
                            ...
                            <p className="text-white text-center"> Media de valoración: {hackathon.avgRating}</p>
